...
describe('GET /api/vendors/export', () => {
  ...
  it('should return 401 if user is not authenticated', async () => {
    ...
    const req = new NextRequest('http://localhost:3000/api/vendors/export');
  ...
  it('should return 200 and CSV data if user has permission', async () => {
    const mockSession: Session = {
      user: { id: 'admin-1', permissions: [Permission.MANAGE_VENDORS], role: { id: 'role-1', name: Role.ADMINISTRATOR } },
      expires: '2025-01-01',
    };
    vi.mocked(getServerSession).mockResolvedValue(mockSession);
